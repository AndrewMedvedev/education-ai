...
from tg_bot.features.user import repository, service
...
async def cmd_start(message: Message) -> None:
    ...
    if user.role == UserRole.TEACHER:
        from tg_bot.features.teacher.keyboards import get_menu_kb

        await message.reply(**WELCOME_TEACHER_TEXT.as_kwargs(), reply_markup=get_menu_kb())
        return
    if user.role == UserRole.STUDENT:
        from tg_bot.features.student.keyboards import get_main_menu_kb
